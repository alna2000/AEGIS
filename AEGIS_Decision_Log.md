...
| Emit typed, allowlisted `LOGIN_SUCCESS` and `LOGIN_FAILURE` events through an authentication audit sink. | Establishes a small application-side audit boundary without arbitrary log strings, unrestricted metadata, or premature Phase 6 persistence. |
| Require authentication audit emission and raise a controlled error if the sink fails. | A login attempt must not appear to complete normally when its required security event was not emitted. |
| Emit successful authentication audit before applying a verifier rehash. | An audit failure leaves the old valid verifier unchanged; the caller retains transaction ownership and no repository helper commits independently. |

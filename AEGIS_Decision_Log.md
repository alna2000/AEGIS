...
| Defer dedicated CSRF tokens while authenticated state-changing browser scope is limited to idempotent logout. | `SameSite=Strict` is useful baseline protection but not universal CSRF protection; additional state-changing authenticated endpoints must not be added before a deliberate CSRF mechanism. |
